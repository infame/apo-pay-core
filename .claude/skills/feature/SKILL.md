...
## Stage 0 — Branch
If the current branch is `master`, create and switch to a feature branch off it before doing anything else: `git checkout -b <feat|fix>/<short-slug>`. If already on a non-master branch, use it as-is (don't create a second branch on top).
...
## Stage 3 — Review
Delegate to `reviewer` AND `security-reviewer` in parallel, on `git diff master...<branch>` (the branch's full changes since it diverged from master, not just the last commit). Give each: the plan and the implementer's final report.
...
- Any REQUEST_CHANGES → collect all Critical/high items, send them to `implementer` (resume), re-run `test-runner`, then re-run BOTH reviewers on the new `git diff master...<branch>`. Max 3 review rounds total; after that stop and report to the user with the unresolved items.
...
- the exact squash-merge command they can run, or do it yourself if the user asked for that in the task: `git checkout master && git merge --squash <branch> && git commit`

Rules:
- Pass context explicitly in every delegation: subagents do not see this conversation.
- Never let implementer merge into master. Never squash-merge into master without `./scripts/approve.sh <branch>` having run on the current `git diff master...<branch>` — `.githooks/reference-transaction` will block the landing commit either way, but don't rely on that; get the review first.
