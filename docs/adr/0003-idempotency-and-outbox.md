...
Accepted (amended 2026-07-11: the outbox _dispatcher_ is descoped from
...
key + a _different_ body is rejected (`IdempotencyConflictError`). See
...
the aggregate state _and_ its domain events in one transaction. Either both
commit or neither does. This removes the dual-write hazard on the _write_ side:
we only ever write to our own database inside the request.

The _delivery_ half — a background dispatcher relaying outbox rows to
