...
# actual diff it records against main, before letting anything land on
# main — regardless of whether that lands via `git merge --squash` +
# `git commit`, or anything else.
# Called by the coordinator after both reviewers return VERDICT: APPROVE on
# `git diff main...<branch>`.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"
TRUNK='main' # keep in sync with .githooks/reference-transaction
