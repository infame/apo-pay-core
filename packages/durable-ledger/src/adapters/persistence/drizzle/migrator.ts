 ...
 *
 * `migrationsSchema: "ledger"` is load-bearing, not cosmetic: it puts this
 * package's migration journal at `ledger.__drizzle_migrations` instead of
 * the driver's default (shared, cross-package) location, so applying
 * durable-ledger's migrations can never mark pay-core's as applied (or vice
 * versa) against the shared Postgres instance. See `schema.ts`'s header
 * comment for the full rationale.
 */
export async function migrate(db: Database): Promise<void> {
  await drizzleMigrate(db, { migrationsFolder, migrationsSchema: "ledger" });
}
